...
$.extend(Datepicker.prototype, {
	...
	_dialogDatepicker: function(input, date, onSelect, settings, pos) {
		var inst = this._dirloginst; // internal instance
		if (!inst) {
			this.uuid += 1;
			var id = 'dp' + this.uuid;
			this._dirloginput = $('<input type="text" id="' + id +
				'" style="position: absolute; top: -100px; width: 0px;"/>');
			this._dirloginput.keydown(this._doKeyDown);
			$('body').append(this._dirloginput);
			inst = this._dirloginst = this._newInst(this._dirloginput, false);
			inst.settings = {};
			$.data(this._dirloginput[0], PROP_NAME, inst);
		}
		extendRemove(inst.settings, settings || {});
		date = (date && date.constructor == Date ? this._formatDate(inst, date) : date);
		this._dirloginput.val(date);
		...
		this._dirloginput.css('left', (this._pos[0] + 20) + 'px').css('top', this._pos[1] + 'px');
		inst.settings.onSelect = onSelect;
		this._inDialog = true;
		this.dpDiv.addClass(this._dialogClass);
		this._showDatepicker(this._dirloginput[0]);
		if ($.blockUI)
			$.blockUI(this.dpDiv);
		$.data(this._dirloginput[0], PROP_NAME, inst);
		return this;
	...
	_hideDatepicker: function(input) {
		...
		if (this._datepickerShowing) {
			...
			if (this._inDialog) {
				this._dirloginput.css({ position: 'absolute', left: '0', top: '-100px' });
				if ($.blockUI) {
					$.unblockUI();
